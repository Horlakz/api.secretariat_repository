...
import (
	...
	models "github.com/horlakz/api.secretariat_repository/model"
...
// FindUserByPhoneNumber implements UserRepositoryInterface.
func (u *userRepository) FindUserByPhoneNumber(phoneNumber string) (user models.User, err error) {
	err = u.database.Connection().Model(&models.User{}).Where("phone_number = ?", phoneNumber).First(&user).Error
