...
import (
	...
	userDto "github.com/horlakz/api.secretariat_repository/dto"
	"github.com/horlakz/api.secretariat_repository/model"
	"github.com/horlakz/api.secretariat_repository/repository"
	userRepository "github.com/horlakz/api.secretariat_repository/repository/user"
)
...
func (service *userService) CreateUser(userDtoArg userDto.UserDTO) (userDto.UserDTO, error) {
	...
	// check if phone number already exists return error
	_, err = service.userRepository.FindUserByPhoneNumber(user.PhoneNumber)
	if !errors.Is(err, gorm.ErrRecordNotFound) {
		return userDto.UserDTO{}, errors.New("phone number already exists")
	}
