...
func (service *authService) Register(authDto dto.AuthDTO) error {
	...
	userDto.PhoneNumber = authDto.PhoneNumber
	userDto.Password = hash
	userDto.IsEmailVerified = false
	userDto.Role = UserRoleUser

	if _, err = service.userService.CreateUser(userDto); err != nil {
		return err
	}
...
func (s *authService) SendEmail(email string, templateType string) error {
	...
	switch templateType {
	case "confirm-email":
		sendEmailParams.Subject = "Thanks for Signing Up on Secretariat Repository"
		_ = s.mail.SendEmail(sendEmailParams)

	case "reset-password":
		sendEmailParams.Subject = "Secretariat Repository Reset Password Request"
		_ = s.mail.SendEmail(sendEmailParams)
	}
